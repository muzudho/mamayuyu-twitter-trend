...
* 📖参考： [King & Prince support for beginners ~はじめてさん🔰への応援ガイド～ > 補講【 トレンド 】トレンドに入る条件](https://kingandprince-support.hatenablog.jp/entry/2021/07/11/010930)
    * `#（ハッシュタグ）＋ひらがな４文字` は、ツイッターのトレンドには入らない（これはよく言われる）
        * だから `#ままゆう` はツイッターのトレンドに**入らない**

* 📖参考：[Wikipedia > Twitterトレンド](https://ja.wikipedia.org/wiki/Twitterトレンド)
    * ４文字未満の単語は、ツイッターのトレンドに入らない（※わたしの解釈：半角英字は除くと思う）
        * だから `魔勇`, `林快彦`, `林先生`  はツイッターのトレンドに**入らない**
    * `#（ハッシュタグ）＋ひらがな４文字` は、ツイッターのトレンドには入らない（これはよく言われる）
        * だから `#ままゆう` はツイッターのトレンドに**入らない**

* わたしの仮説
    * **魔々勇々**, "**#魔々勇々**" は **々** の字が記号・句読点のようなものなので、まるで `魔、勇。` のようなものなので、上記に同じ（かもしれない？）

* 分からないこと
    * **#林快彦** は、シャープ印を含めて４文字以上だからツイッターに載るか、そうでないか　どちらか
...
### （この節は読み飛ばしても構いません）仮説の解説その２　４文字未満の日本語はツイッターのトレンドに入らないかもしれない

ウィキペディアの [Twitterトレンド](https://ja.wikipedia.org/wiki/Twitterトレンド)  という記事を要約すると、

* 公式はルールを明言していないので、利用者の間でもっともらしく言われていることを並べると
    * 3文字の（日本語の）文字列はトレンド入りせず **４文字以上** 必要だろう
    * **#（ハッシュタグ）＋ひらがな4文字** は5文字だが **トレンド入りしない** だろう（※わたしには、これが本当なのかどうか分からない）
...
話し＜その１＞で既述のように、 **＃（ハッシュタグ）＋ひらがな４文字** もツイッターのトレンドには入らないと書かれています。  
（※無間地獄を見ているようだ）  

ただし、 `#それスノ`, `#フロイニ` という日本語４文字の単語はトレンド入りしていた。  
...
日本人が作ったサービスではなく、海外製のサービスを使っているわけで、私企業の製品に　日本の文化をカバーするよう働きかけていないから仕方ありません。
